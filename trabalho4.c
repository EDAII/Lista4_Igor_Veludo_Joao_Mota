...
//const int SIZE_NUMBERS = 100;

void imprimeGrafico(int *vetor, int size){
	int linha, coluna;
  char matriz[MAX+1][MAX+1];
  //prenche todos os espacos da matriz com espacos em brancos
...
void randomize_input(int *input_numbers, int size){
	...
	// numbers are generated from 0 to size_NUMBER-1
	const int size_NUMBER = size * 5;

	// array to mark already generated numbers
	int generated[size_NUMBER];
...
void partition_numbers_around_mom (int *input_numbers,
	...
	imprimeGrafico(input_numbers, input_numbers_size);
}
...
int exoteric_select (int *input_numbers, int input_numbers_size, int k_esimo) {
	...
	if (mom_index > expected_size) {
		...
		imprimeGrafico(left_partition, left_size);
		...
		imprimeGrafico(right_partition, right_size);
...
int main(int argc, char *argv[]) {
    ...
    printf("Escolha o tamanho do vetor entre 1 e %d: ", MAX);
		scanf("%d", &choose);
		while(choose > MAX || choose <= 1){
    ...
    input_numbers_size = choose;
    assert(input_numbers_size > 0);
    input_numbers = (int*) malloc(sizeof(int)*(input_numbers_size));
    randomize_input(input_numbers, input_numbers_size);
    aux = partition_copy(input_numbers, 0, input_numbers_size - 1);
    ...
    print_numbers(input_numbers, input_numbers_size);
		...
		k_esimo = (input_numbers_size - 1) / 2; // median is at middle
    ...
    median = exoteric_select(input_numbers, input_numbers_size, k_esimo);

      print_numbers(aux, input_numbers_size);
			imprimeGrafico(aux, input_numbers_size);
